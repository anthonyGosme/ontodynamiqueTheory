/-!
=== TEST H2a : cost_decomposition RESTAURÉE, regen_pos AFFAIBLIE ===

Méthode : copie de v4.4 avec MetabolizingClosure affaiblie.
  regen_pos → regen_any : True
  cost_decomposition → cost_decomposition : drain_net + regeneration = total_cost
Bruit syntaxique corrigé (champs renommés).
Les erreurs de compilation = dépendances SÉMANTIQUES à I-β.

PRÉDICTION H2a : 49 PASSENT / 3 ÉCHOUENT
  Échecs : XXXVIII-a, XXXVIII-b (transitif), XXXIX-a
  Récupère : XXXVIII-d (is_endogenous) grâce à cost_decomposition + drain_net_pos
-/
...
theorem metabolization_reduces_drain (m : MetabolizingClosure) :
  ...
  have := m.cost_decomposition; have := m.regen_any; omega
...
/-- [H2a — RÉCUPÈRE] XXXVIII-d
    cost_decomposition restaurée → omega dérive :
    drain_net + regen = total_cost ∧ drain_net ≥ 1
    → regen = total_cost - drain_net ≤ total_cost - 1 < total_cost -/
...
/-- [AUDIT I-β — ÉCHEC ATTENDU] XXXIX-a
    m.regen_any : True, but goal is m.regeneration > 0 → TYPE MISMATCH
    Sans regen_pos, rien ne garantit que la régénération est non-nulle -/
theorem normativity_criterion (m : MetabolizingClosure) :
    m.regeneration > 0 := m.regen_any
